...
public class TetrisGUI extends JPanel implements Observer {
    ...
    private void setSoundEffect() {
        try {
            final AudioInputStream audioIn = AudioSystem.getAudioInputStream(new File("his"
                            + "ticks.wav"));
    ...
    private void addMenuBar() {
        ...
        final JMenuItem endGame = new JMenuItem(new AbstractAction() {
            ...
            public void actionPerformed(final ActionEvent theEvent) {
                ...
                int answer = 0;
                answer = JOptionPane.showConfirmDialog(null, "Game ended! Play again?"
                                + "\n (No to quit)"
                                                       , GAME_OVER, type);
    ...
    private void addMenuBar2() {
        ...
        final JMenuItem controlsItem = new JMenuItem(new AbstractAction() {
            ...
            public void actionPerformed(final ActionEvent theActionEvent) {
                ...
                JOptionPane.showMessageDialog(myGameWindow, helpMsg
                                              , "Help - Controls", helpType);
                resume();
            }
        });
    ...
    private void setupInnerPanels() {
        ...
        nextLabel.setBackground(Color.YELLOW);
        ...
        // setup colors;
        myBoardPanel.setBackground(Color.YELLOW);
    ...
    private void setUpAudio() {
        ...
        try {
            final AudioInputStream audioIn = AudioSystem.getAudioInputStream(new File("tetris"
                            + "-theme.wav"));
     ...
     */
    private void pause() {
        // pause game by stopping the timer and setting the boolean myPaused to true
        setPaused(true);
        myTimer.stop();
        if (myClip.isRunning()) {
            myClip.stop();
     ...
     */
    private void resume() {
        // resume game by starting the timer and setting the boolean myPaused to false
        setPaused(false);
        myClip.start();
        myTimer.start();
    }
    ...
    public void update(final Observable theObj, final Object theArg) {
        if (theObj instanceof Board && theArg instanceof Board.GameStatus) {
            ...
            if (gameOver) {
                ...
                myAnswer = JOptionPane.showConfirmDialog(this, "You're bad!\nWould you "
                                    + "like to play again?"
                                                           , GAME_OVER, type);
